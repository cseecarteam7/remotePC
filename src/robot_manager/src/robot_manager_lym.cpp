...
void automove(double avg, ros::Publisher &pubTeleop, bool &stuck_flag) 
{
	...
	if(obstacle_flag){
		...
		printf("[robot_manager/automove] STUCK IN!\n");
...
bool comparePosition(nav_msgs::Odometry &odom_before, nav_msgs::Odometry &odom_after){
	
	bool stuck_flag = false;

	geometry_msgs::Point translation_before = odom_before.pose.pose.position;
	geometry_msgs::Quaternion rotation_before = odom_before.pose.pose.orientation;

	geometry_msgs::Point translation_after = odom_after.pose.pose.position;
	geometry_msgs::Quaternion rotation_after = odom_after.pose.pose.orientation;

	double tx, ty, tz;
	double rx, ry, rz, rw;

	tx = fabs(translation_before.x - translation_after.x);
	ty = fabs(translation_before.y - translation_after.y);
	tx = fabs(translation_before.z - translation_after.z);

	rx = fabs(rotation_after.x - rotation_before.x);
	ry = fabs(rotation_after.x - rotation_before.x);
	rz = fabs(rotation_after.x - rotation_before.x);
	rw = fabs(rotation_after.x - rotation_before.x);

	if(tx < 0.05 && ty < 0.05 && tx < 0.05 ){
		stuck_flag = true;
	}
...
int main(int argc, char **argv)
{
	...
	//vector< vector<double> > point_arr;
	//vector< vector<double> > quaternion_arr;
	...
	nav_msgs::Odometry odom_after;
	bool stuck_flag = false;

	odom_before = odom;
	ros::Time timestamp_before = odom_before.header.stamp;
...
/*		if((timestamp_before.sec < timestamp_after.sec) && abs(timestamp_after.sec - timestamp_before.sec) < 6){
		...
		else if((timestamp_before.sec > timestamp_after.sec) && ((60 - timestamp_before.sec + timestamp_after.sec) < 6 )){
			odom_after = odom;
			timestamp_after = odom_after.header.stamp;
			printf("#### else if 2222 ####\n");
		}
...
*/
		...
		if(catDetectFlag == false){
			...
			automove(avg, pubTeleop, stuck_flag);
